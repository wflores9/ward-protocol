...
const STEPS = [
  { n: 1, name: 'NFT Existence & Taxon', desc: 'Policy NFT in claimant wallet with taxon = 281 (WARD_POLICY_TAXON)',
    detail: 'Ward calls AccountNFTs on the claimant address and verifies at least one NFT exists with NFTokenTaxon = 281. This taxon is the WARD_POLICY_TAXON constant â€” any NFT with a different taxon is rejected regardless of other attributes.' },
  { n: 2, name: 'Policy Expiry', desc: 'Expiry checked against XRPL ledger close_time â€” not server clock',
  ...
  { n: 4, name: 'On-Chain Default Flag', desc: 'LedgerEntry carries LSF_LOAN_DEFAULT (0x00010000)',
    detail: 'Ward fetches the LedgerEntry for the reported loan and checks that the LSF_LOAN_DEFAULT flag (0x00010000) is set. This flag is set by the XLS-66 vault contract upon confirmed default â€” it cannot be set off-chain.' },
  ...
  { n: 6, name: 'Pool Coverage Breach', desc: 'Usable pool balance = balance âˆ’ XRPL reserve â‰¥ 0',
  ...
  { n: 7, name: 'Replay Protection', desc: 'NFT still live on-chain; burn-on-settlement prevents replays',
    detail: 'Ward confirms the policy NFT is still present in the claimant wallet at claim time. Upon successful settlement the NFT is burned via NFTokenBurn. A burned token cannot be replayed â€” the same loss event cannot be claimed twice.' },
  { n: 8, name: 'Claimant Holds NFT', desc: 'NFT found in claimant_address wallet via AccountNFTs',
    detail: 'Ward calls AccountNFTs on the claimant_address and verifies the specific NFTokenID is present. The NFT must be in the claimant wallet â€” not escrowed, not in another account. This binds the claim to the actual policy holder.' },
  { n: 9, name: 'Pool Solvency & Rate Limit', desc: 'Usable â‰¥ payout, ratio â‰¥ 1.5Ã—, â‰¤ 3 claims/NFT per 300 s',
    detail: 'Three final checks: (1) usable pool balance must cover the full payout amount, (2) post-payout pool ratio must remain â‰¥ 1.5Ã— to prevent pool insolvency, (3) no more than 3 claims per NFT within any 300-second window to prevent automated draining attacks.' },
]
  ...
  return (
    <div className="max-w-4xl mx-auto px-6 md:px-12 py-12 grid md:grid-cols-[1fr_280px] gap-8 items-start">
      ...
      <div>
        ...
        <p className="text-sm text-sub mb-6">All state sourced from XRPL ledger â€” no off-chain inputs trusted.</p>
          ...
          <ul className="space-y-2 text-sm text-sub">
            {[
              ...
              <li key={k} className="flex gap-2">
                ...
                <span><strong className="text-steel">{k}</strong> â€” {v}</span>
              </li>
            ))}
          </ul>
        </div>
