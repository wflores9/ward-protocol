...
CREDENTIAL_NFT_TAXON   = 282          # Ward credential taxon — distinct from policy taxon 281
...
# ---------------------------------------------------------------------------
def build_kyc_hash(depositor_address,institution_address,xrpl_ledger_time,kyc_type):
    """Build deterministic KYC commitment hash: sha256(addr|addr|time|type)."""
    raw = f"{depositor_address}|{institution_address}|{xrpl_ledger_time}|{kyc_type}"
    return hashlib.sha256(raw.encode()).hexdigest()

def validate_kyc_hash(kyc_hash):
    """Validate kyc_hash is 64-char lowercase hex (sha256)."""
    if not isinstance(kyc_hash, str):
        raise ValidationError(f"kyc_hash must be str, got {type(kyc_hash).__name__}")
    if not re.fullmatch(r"[0-9a-f]{64}", kyc_hash):
        raise ValidationError(f"kyc_hash must be 64-char hex sha256. Got: {kyc_hash[:16]!r}...")
...
class WardClient:
    ...
    async def issue_credential(
        self,
        institution_wallet: Any,
        depositor_address: str,
        kyc_type: str,
        period_days: int,
        kyc_record_hash: str | None = None,
    ) -> Dict[str, Any]:
        """Issue an XLS-70 on-chain credential NFT to a depositor wallet.

        Ward validates inputs and builds the unsigned NFTokenMint.
        Institution signs and submits — ward_signed = False always.

        Returns dict with credential_id, nft_token_id, kyc_hash, status.
        """
        validate_xrpl_address(institution_wallet.classic_address, "institution_address")
        ...
        try:
            async with AsyncWebsocketClient(self._xrpl_url) as client:
                ...
                current_ledger_time = ledger_result.result["ledger"]["close_time"]
                expiry_ledger_time = current_ledger_time + (period_days * 86400)
                if kyc_record_hash is None:
                ...
                uri_data = json.dumps({
                    ...
                    "kyc_type": kyc_type, "kyc_hash": kyc_hash,
                    "depositor": depositor_address,
                    "issued_at": current_ledger_time,
                    "expires_at": expiry_ledger_time,
                }, separators=(",", ":"))
                uri_hex = str_to_hex(uri_data)
                if len(uri_hex) > MAX_CREDENTIAL_URI_LEN * 2:
                    raise ValidationError(f"Credential URI {len(uri_hex)} chars exceeds limit.")
                # ward_signed = False — institution signs, Ward never does
                credential_tx = NFTokenMint(
                    account=institution_wallet.classic_address,
                    nftoken_taxon=CREDENTIAL_NFT_TAXON,
                    flags=8,
                    transfer_fee=0,
                    uri=uri_hex,
                    memos=[Memo(
                        ...
                        memo_data=str_to_hex(json.dumps({
                            "kyc_type": kyc_type, "depositor": depositor_address,
                            "issued_at": current_ledger_time,
                        }, separators=(",", ":"))),
                    )],
                )
                autofilled = await autofill(credential_tx, client)
                signed = sign(autofilled, institution_wallet)
                result = await submit_and_wait(signed, client)
                tx_result = result.result.get("meta", {}).get("TransactionResult")
                if tx_result != "tesSUCCESS":
                    raise LedgerError(f"Credential NFT mint failed: {tx_result}")
                ledger_tx_hash = result.result.get("hash", "")
                nft_token_id = ""
                for node in result.result.get("meta", {}).get("AffectedNodes", []):
                    created = node.get("CreatedNode", {})
                    if created.get("LedgerEntryType") == "NFToken":
                        nft_token_id = created.get("NewFields", {}).get("NFTokenID", "")
                        break
                return {
                    "credential_id": f"cred_WRD_{kyc_hash[:8]}",
                    "nft_token_id":  nft_token_id,
                    "ledger_tx":     ledger_tx_hash,
                    "depositor":     depositor_address,
                    "kyc_type":      kyc_type,
            ...
            raise LedgerError(f"issue_credential failed: {exc}") from exc
...
class ClaimValidator:
    ...
    async def _validate_credential_kyc_hash(
        ...
        """Step 1b: Verify depositor holds a valid Ward credential NFT (taxon 282).
        ...
        try:
            ...
            credential_nfts = [n for n in nfts if n.get("NFTokenTaxon") == CREDENTIAL_NFT_TAXON]
            if not credential_nfts:
                return False
            ledger_result = await self._client.request(Ledger(ledger_index="validated"))
            current_time = ledger_result.result["ledger"]["close_time"]
            for nft in credential_nfts:
                uri_hex = nft.get("URI", "")
                if not uri_hex:
                    continue
                try:
                    uri_data = json.loads(hex_to_str(uri_hex))
                except Exception:
                    continue
                if uri_data.get("expires_at", 0) < current_time:
                    continue
                try:
                    validate_kyc_hash(uri_data.get("kyc_hash", ""))
                except ValidationError:
                    continue
                if uri_data.get("depositor") != depositor_address:
                    continue
                return True
            return False
        except Exception:
